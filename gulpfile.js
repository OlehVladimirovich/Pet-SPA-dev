...
gulp.task('watch', function(){      //Следим за файлами и сразу меняем, удаляет dist после перезапуска
    gulp.watch('./src/sass/**/*.{sass,scss}', gulp.parallel('sass'));  // Файлы sass
    ...
    gulp.watch('./src/js/**/*.js', gulp.parallel('js'));  // Добавлено отслеживание JS файлов
    gulp.watch('./src/css/**/*', gulp.parallel('css'));
});

gulp.task('default', gulp.series(
    'clean',
    gulp.parallel('html', 'sass', 'js', 'css', 'images', 'fonts', 'files'),
